...
def test_bidding_system(results: TestResults):
    ...
    if "admin" in results.tokens and "bid_id" in results.test_data:
        ...
        success, response, status = make_request("PUT", f"/bids/{bid_id}/status?status_value=accepted", token=results.tokens["admin"])
        if success:
        ...
        if success and "id" in response:
            bid_id2 = response["id"]
            success, response, status = make_request("PUT", f"/bids/{bid_id2}/status?status_value=rejected", token=results.tokens["admin"])
            if success:
